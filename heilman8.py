...
class Graph(object):
    ...
    def Adj(self):
        x = len(self.Graph)
        matrix = np.zeros(shape = (x,x), dtype=np.int)        #create matrix with correct dimensions
        
        items = []                  
        
        for item in self.Graph:     # list of vertices used for indexing
            items.append(item)

        
        index = -1                                   #set to -1 so don't get index error
        for item in self.Graph:                   # look at each vertex in self.Graph
            index = index + 1                        #add 1 to index for adj matrix
            for j in range(len(self.Graph[item])):       #look at each adj vertex
                for i in range (len(items)):                 #find index of adj vertex
                    if self.Graph[item][j] == items[i]:
                        matrix[index][i] = 1
                       # matrix[i][index] = 1            #unnecessary  because for loop checks every vertex
    ...
    def Deg(self):
        ...
        for item in self.Graph:     #look at every vertex
            index = index + 1
            matrix[index][index] = len(self.Graph[item])    #make diagnal the amound of adj vertices
    ...
    def Inc(self):
        ...
        matrix = np.zeros(shape = (n,m), dtype=np.int)                        #matrix with dimension (vertices x edges)

        n = len(self.vertices())                      # of vertices
        m = len(self.edges())                        # of edges
            
        edges = []
        for i in self.edges():                       #make list ofedges for indexing purposes
            edges.append(i)
                        
                        
        index = -1                                        #set to -1 so don't get index error
        
        for item in self.Graph:                                          #look at each vertex
            index = index + 1
            for adj in self.Graph[item]:                                 #look at adjacent vertices
                for i in range (len(edges)):                                 #find index of edge
                    if ((item,adj) == edges[i] or (adj,item) == edges[i]):  #checking for edge (1,2) and (2,1)
                        matrix[index][i] = 1                                 #add one to represent incidence
    ...
    def Dist(self):
        ...
        n = len(self.Graph)             #amount of vertices in graph
        
        adj = self.Adj()                #adjacency matrix
        distance = adj                  #final distance matrix, adj vertices already added
        adj2 = adj
        
        for i in range (n-2):                #n-1 since it's the max distance between 2 vertices
            adj = adj2                        #reset to original asj matrix each iteration
            adj = np.asmatrix(adj)               #convert adj to a matrix from 2d array, needed for matrix multiplication
            adj = adj**(i+2)                      #create A^2, A^3 .... ,A^n-1 
            adj = np.asarray(adj)                 #convert to array for indexing
            for j in range (n):
                for k in range (n):                             #j != k makes diagonal unchanged
                    if distance[j][k] == 0 and j != k and adj[j][k] != 0:         #if distance matrix has a 0, change it with smallest distance
                        distance[j][k] = i+2        #set to i+2 because A^0 and A^1 have already been checked
                                                      #so when i = 0, it really equals 2
...
#def main():
   ...
   # G = Graph()
    ...
    #G.add_edges([(1,2),(1,5),(1,4),(2,3),(2,6),(3,5),(3,4),(4,5)])
  #  print(G.Adj())
    #print(G.Deg())
   # print(G.Inc())
   # print(G.Dist())
